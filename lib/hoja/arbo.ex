defmodule Hoja.Arbo do
  ...
  def get_parents!(id, entity_type) do
    cond do
      entity_type == "network" ->
        Ecto.Adapters.SQL.query!(Hoja.Repo, "SELECT id from entities WHERE parent_id = ? AND entity_type = 'school'" , [id])
      entity_type == "school" ->
        Ecto.Adapters.SQL.query!(Hoja.Repo, "SELECT id from entities WHERE parent_id = ? AND (entity_type = 'network' OR entity_type = 'class')", [id])
      entity_type == "class" ->
        Ecto.Adapters.SQL.query!(Hoja.Repo, "SELECT id from entities WHERE parent_id = ? AND entity_type = 'school'" , [id])
    end
